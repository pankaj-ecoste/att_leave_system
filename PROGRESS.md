...
✅ DONE      Flexible work-window rule for stdHours (2026-08-07, your call, frontend-only — no migration): staff can punch in any time, but the 9-hour requirement now has to fit inside a fixed **9:00 AM–7:00 PM window**. A 9:00 punch-in still has slack (9am-6pm covers 9h); a 10:00 punch-in's window ends exactly at 7pm; any later than that and 9h can no longer fit. New `WORK_WINDOW_START`/`WORK_WINDOW_END` in `constants.js`, `calcStatus` in `lib/datetime.js` caps hours worked past 7pm and no longer counts them — **except** a late punch-in who stays through to 7pm anyway (used their whole available window) is still marked **Present**, not demoted to Half Day/Absent, per your call; a late punch-in who *also* leaves early still falls to the normal Half Day/Absent thresholds. **Night Shift is untouched** — the window rule only applies when punch-in falls before 7pm, so a 9pm night-shift punch-in never enters this logic at all, same plain calculation as before. The Present-but-short case is surfaced separately via a new `hasIncompleteHoursFlag()` — shows an "⚠ incomplete hrs" note next to Status in Admin → Attendance → Daily Records, and a new "Incomplete hours (late punch-in)" row in the Daily Report's Exceptions sheet — so nobody's status looks penalized automatically but admin/HR still sees it, same posture as the existing LOP/absence report pattern. 5 new vitest cases (37 total in `datetime.test.js`) cover the window boundary, the forgiveness case, the early-leave-still-Absent case, and Night Shift being unaffected — all passing. `npm run build` clean, G-1 guardrail clean (no schema touched, frontend-only change). **Not yet browser-verified** — build/tests prove the logic, but nobody has watched the new badge/exception row render on a real screen yet.
