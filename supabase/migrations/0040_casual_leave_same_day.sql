-- HR request (2026-09-09): Casual Leave (full-day and half-day/first-half/second-half)
-- can be applied for today, not just future dates. employee_apply_leave requires every
-- leave type except Sick/Bereavement/WFH/On Duty/LOP/Earned Leave/Partial Leave to be
-- applied at least 1 day in advance (v_date < current_date + 1 -> rejected). Casual
-- Leave was never added to that exemption list when Partial Leave was (0031), so staff
-- hit "Casual Leave must be applied at least a day in advance" trying to apply the same
-- day it's needed — same class of bug 0031 already fixed for Partial Leave. Half-day is
-- only available for Sick/Casual/Earned Leave, but Sick Leave is already exempt and
-- Earned Leave has its own stricter 7-day rule, so Casual Leave (full or half-day) was
-- the only type actually affected.
--
-- Fix: give Casual Leave the same same-day-allowed treatment Partial Leave already has
-- — today and any future date are allowed, only genuinely past dates are rejected.
-- Every other rule in this function (probation cap, half-day eligibility, 18-month
-- service check, balance check, Plant restriction, duplicate-application check) is
-- untouched — see plan.md §22.
