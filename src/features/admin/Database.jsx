          ...
          <div>
            ...
            {locLogs.length === 0 ? (
              ...
              <table className="w-full text-xs text-white/60 min-w-max">
                ...
                <tbody>{locLogs.filter(r => !q || `${r.empName} ${r.empNum} ${r.siteName || ''} ${r.latLon} ${r.type}`.toLowerCase().includes(q)).map((r, i) => (
                  <tr key={i} className="border-b border-white/5 hover:bg-white/5">
                    ...
                    <td className="px-3 py-2 max-w-[250px] truncate text-purple-400/80">{r.siteName || r.latLon}</td>
